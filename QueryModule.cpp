...
QueryModule::QueryModule(QTcpSocket *sharedSocket, const QString &analyticsLeaderAddress, QObject *parent)
    : QObject(parent), socket(sharedSocket), analyticsLeaderAddress(analyticsLeaderAddress)
{
...
void QueryModule::sendQuery()
{
    ...
    if (socket->state() == QAbstractSocket::ConnectedState || socket->waitForConnected(5000))
    {
        ...
        {
            std::cout << "Query sent to metadata analytics leader: " << analyticsLeaderAddress.toStdString() << std::endl;
        }
        else
        {
